...
class ObstacleDetector(Node):
    ...
    def load_robot_configurations(self):
        yaml_file = os.path.join(get_package_share_directory('inter_iit'), 'config', 'robots.yaml')
        ...
        for robot_list in robots_config.values():
            for robot in robot_list:
                ...
                self.create_subscription(LaserScan, f'/{robot_name}/{robot_name}/scan', lambda msg, topic=robot_name: self.laser_callback(msg, topic), 10)
